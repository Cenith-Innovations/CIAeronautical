...
public struct WX {
    ...
    public static func getWx(wxString: String, ceilingString: String, wind: Double, lat: Double?, long: Double?) -> WxCondition {
        ...
        if let lat = lat, let long = long {
            let suntime = SunriseSunset(lat: lat, long: long)
            let sunrise = suntime.sunRise?.timeIntervalSinceReferenceDate
            let sunset = suntime.sunSet?.timeIntervalSinceReferenceDate
            let now = Date().timeIntervalSinceReferenceDate
            if let sunrise = sunrise, let sunset = sunset, now >= sunrise, now < sunset {
