...
public class OpenSkyAPI: ObservableObject {
    ...
    public static func getAdsBResults(type: SearchType, time: Int? = nil) {
        let url = OpenSkyWebAPI.url(type: type, time: time)
        let request = URLRequest(url: url)
        let task = session.dataTask(with: request) { (data, response, error) in
            if let adsBResults = data {
                loadJSONintoStore(adsBResults)
    ...
    private static func loadJSONintoStore(_ data: Data) {
        ...
        OpenSkyAPI.shared.store.send(currentTraffic)
        OpenSkyAPI.shared.foreFlightTraffic.send(foreFlightFormatTraffic)
    }
